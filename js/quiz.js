jQuery(function ($) {
  ...
  localStorage.setItem("addedSymptoms", JSON.stringify(addedSymptoms, null, 2));
  ...
  function getSlide3(probability) {
    $("#slide-3-probability").text(probability);
  ...
  function getSlide4(probability) {
    $("#slide-4-probability").text(probability);
  ...
  function searchNext() {
    ...
    if ($("#search-output").html() == "") {
      $("#nothing-found").show();
      $("#nothing-found").text("К сожалению такого симптома нет в нашей базе");
    } else {
      $("#nothing-found").hide();
    }
